            ...
            {currentUsers.map((user) => (
              <tr key={user.userId} className="border-t border-gray-200 hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-2">
                  <span
                    className={`inline-block px-2 py-1 text-xs font-semibold rounded-full ${user.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}
                  >
                ...
                <td className="p-4 space-x-2 font-medium text-blue-600">
                  <NavLink className="action-link" to="/admin/users/update">Edit</NavLink>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mt-4 p-4 border-t border-gray-200 gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`cursor-pointer flex size-8 items-center justify-center rounded-md border border-slate-300 transition-colors ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-100'
                }`}
            >
              <MdKeyboardArrowLeft />
            </button>
            {getPageNumbers().map((page) => (
              <button
                ...
                className={`cursor-pointer text-sm font-medium flex size-8 items-center justify-center rounded-md transition-colors ${page === currentPage ? 'text-white bg-blue-600' : 'text-slate-600 hover:bg-slate-100'
                  }`}
              >
                {page}
              </button>
            ...
            <button
              ...
              className={`cursor-pointer flex size-8 items-center justify-center rounded-md border border-slate-300 transition-colors ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-100'
                }`}
            >
